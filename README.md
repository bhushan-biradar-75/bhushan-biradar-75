<h1 align="center">Hey 👋, I'm Bhushan</h1>

---

### 🚀 About Me

- 👨‍💻 Frontend Developer with **4+ years of experience**
- ⚛️ Strong in **React.js, Next.js & JavaScript**
- 🏗️ Built scalable applications in **Fintech & Automobile domain**
- 💡 Passionate about clean architecture, system design & performance
- 🎯 Currently improving DSA, system design & machine coding for product companies
- 📚 Love exploring how things work under the hood — not just using them
- 💬 I enjoy helping and mentoring developers whenever possible
- 🧠 Preparing for Product-Based Frontend roles (React + Next.js ecosystem)

---

### 🛠️ Tech Stack & Tools

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,nodejs,redux,git,github,figma,vscode" />
</p>

---

### 🔥 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical" width="45%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=radical" width="45%" />
</p>

---

### 📌 Featured Projects

| Project | Tech | Status | Link |
|--------|------|--------|------|
| ⚡ Fintech Dashboard Clone | React, Chart.js, Firebase | 🚧 In Progress | Coming Soon |
| 🛒 Ecommerce UI | React + Redux Toolkit | ✔️ Completed |  Soon |
| 🎵 Music Player | React + Custom Hooks | 🚧 Improving | Soon |

---

### 🎙 Fun Stuff

- 🧪 Trying startup ideas with patience and curiosity
- 🛠️ Experimenting with PWA, Web performance & Next.js routing strategies
- 🌍 Love attending React meetups & dev community events
- 🐶 Celebrate birthdays with dogs (yes, that’s my vibe 🫶)

---

### 📬 Connect With Me

<p align="left">
<a href="https://linkedin.com/in/YOUR_LINKEDIN">
  <img src="https://skillicons.dev/icons?i=linkedin" width="40"/>
</a>
<a href="mailto:YOUR_EMAIL@gmail.com">
  <img src="https://skillicons.dev/icons?i=gmail" width="40"/>
</a>
<a href="https://github.com/YOUR_USERNAME">
  <img src="https://skillicons.dev/icons?i=github" width="40"/>
</a>
</p>

---

⭐ **If you find my repositories useful — drop a star. It helps 😊**
